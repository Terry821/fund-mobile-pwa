(() => {
  ...
  function getOfficialNavStatus(fund) {
    ...
    const today = chinaDateStr();
    // QDII/跨境基金常见情况：25 号晚上公布的是 22 号净值。
    // 这时“净值日”落后估值日，但它仍然是今天刚公布的官方最新净值，应该主展示；只是不能当作 25 号涨幅。
    const publishedToday = hasNav && !!publishDate && compareDate(publishDate, today) === 0;
    const staleButFresh = stale && publishedToday;
    return {
      ready,
      stale,
      staleButFresh,
      ...
      reason: ready ? 'official_current' : staleButFresh ? 'official_lagged_fresh' : stale ? 'official_lagging_old' : 'official_missing',
    };
  }
  ...
  function getDisplayMode(fund) {
    ...
    if (primary.staleOfficialOnly) {
      ...
      return {
        ...
        value: nav(primary.nav),
        pct: null,
        sub: `净值日 ${officialStatus.navDate || '--'} · ${lag}`,
        badge: '净值滞后',
        source: 'stale',
      };
    }
    const staleText = officialStatus.stale ? ` · 净值滞后至 ${officialStatus.navDate || '--'}` : '';
    return {
      title: '盘中估值',
      value: nav(primary.nav),
      pct: primary.pct,
      sub: `${pct(primary.pct)} · ${primary.source}${staleText}`,
      badge: officialStatus.stale ? '净值滞后' : '盘中估值',
      source: 'estimate',
    };
  }
  ...
  function renderSummary() {
    ...
    $('summaryGrid').innerHTML = [
      ...
      summaryCard('收益变化', hasToday ? money(totalToday) : '--', 'QDII滞后净值显示净值日收益', clsByNum(totalToday)),
    ].join('');
  }
  ...
  function renderFundCard(fund) {
    ...
    const officialBadge = pos.officialReady
      ...
      : pos.officialLagged
        ? '<span class="mode-badge official lagged">净值已更新</span>'
        : navStatus.stale
          ? '<span class="mode-badge stale">净值滞后</span>'
    ...
    const performanceSub = pos.officialReady ? '官方涨幅' : pos.officialLagged ? `净值日 ${navStatus.navDate || '--'} 涨幅` : primary.staleOfficialOnly ? '净值滞后' : '估算涨幅';
    const navFreshnessText = pos.officialReady
      ? '已覆盖为官方净值'
      : pos.officialLagged
        ? `今天更新 ${navStatus.navDate || '--'} 净值，未覆盖估值日 ${navStatus.targetDate || '--'}`
        : navStatus.stale
          ? `官方净值滞后至 ${navStatus.navDate || '--'}，估值日 ${navStatus.targetDate || '--'}`
          : `官方净值未出 · 上次净值 ${pct(fund.dailyGrowth)}`;
    return `
  ...
  function renderFundDetail(fund, context) {
    ...
    const navFreshness = pos.officialReady
      ...
      : pos.officialLagged
        ...
        : navStatus.stale
          ? `净值日 ${navStatus.navDate || '--'} 落后估值日 ${navStatus.targetDate || '--'}，若不是今天刚更新，则只作参考`
          : `等待官方净值覆盖估值日 ${navStatus.targetDate || '--'}`;
    return `
      <div class="fund-detail expanded-detail">
        ...
        <div class="valuation-box detail-box">
          <div class="detail-head">
            <div>
              <h4>估值拆分</h4>
              <p>官方净值分“净值日”和“发布时间”：QDII 如果今天公布了更早交易日净值，会主展示最新官方净值；但不会把它当作今天的涨幅。</p>
            </div>
          </div>
          <div class="detail-metrics">
            ${metric('最新净值', nav(fund.unitNav), clsByNum(fund.dailyGrowth), `${pct(fund.dailyGrowth)} · ${fund.unitNavDate || '--'}`)}
            ${metric('净值状态', pos.officialReady ? '已覆盖' : pos.officialLagged ? '已更新/滞后' : navStatus.stale ? '滞后' : '未出', navStatus.stale && !pos.officialLagged ? 'warn-text' : '', navFreshness)}
